---
title: "Note: Continuous SEM, with frequentist random intercept modeling with heteroscedasticity"
...
We do so via an exponential term. We fitted three models: 1. linear effect of Xplus; 2. quadratic effect of Xplus; 3. linear plus quadratic effect of Xplus.
For each of the three models, the following is shown:
** The estimates of cSEM per sum score (0 to 10) are printed in a table.
** A residual plot - plotting the fitted values (including the random intercepts) by the normalized residuals. 
...
model <-  try(lme(fixed = Score ~ Item, random = ~1 | Person, weights = varExp(form = ~ Xplus), data = Xlong)
    , silent = TRUE)
 if (inherits(model, "try-error")) {
   sam_estcSEMs_1M[1,]  <- matrix(nrow = 1, ncol = nJ+1, NA)
  } else {
  fit[1,] <- c(logLik(model),BIC(model))
  sam_estcSEMs_1M[1,] <- eval_resid_var(model, Xplus_eval) 
  print(plot_resid_var(model, Xlong$Xplus, title = "linear effect for varEXp"))
}
...
if (inherits(model, "try-error")) {
   sam_estcSEMs_1M[2,]  <- matrix(nrow = 1, ncol = nJ+1, NA)
} else {
 fit[2,] <- c(logLik(model),BIC(model))
  sam_estcSEMs_1M[2,] <- eval_resid_var(model, Xplus2_eval)
  print(plot_resid_var(model, Xlong$Xplus, title = "quadratic effect for varEXp"))
}
...
model <-  try(lme(fixed = Score ~ Item, random = ~1 | Person, weights = varComb(
    ...
    , silent = TRUE)
 if (inherits(model, "try-error")) {
    sam_estcSEMs_1M[3,]  <- matrix(nrow = 1, ncol = nJ+1, NA)
  } else {
fit[3,] <- c(logLik(model),BIC(model))
 sam_estcSEMs_1M[3,] <- eval_resid_var(model, Xplus_eval, Xplus2_eval) 
...
rownames(fit) <- c("lme_lin","lme_qua","lme_lin+qua" )
colnames(fit) <- c("logLik","BIC" )
print(kable(fit, caption = "Fit",digits = 3))

best_model <- rownames(fit)[which.min(fit$BIC)]
...
The model with the lowest  BIC is **`r best_model`**. 
The residual plots show two about linear bands. The two bands are due to the fact that we model binary observed scores, with a model with an assumption of normality of residuals. The direction of the bands puzzles me, that is: I would prefer to see a horizontal band.
...
* Willen we nog flexibelere variantiefuncties (splines, smooths), dan lijkt het erop dat we Bayesiaans moeten gaan, via brms, of Stan. Daarover gaat de volgende Note.
