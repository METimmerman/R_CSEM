...
rownames(sam_estcSEMs_1M) <- c("cSEM_lme_1","cSEM_lme_2","cSEM_lme_3" )
...
resid_var <- (sigma_hat^2 * exp(2 * gamma_hat * Xvals_Xplus)) # stap 4: model-implied residual variances 
...
sigma_hat <- model$sigma
gamma_hat <- coef(model$modelStruct$varStruct, unconstrained = FALSE) # stap 2: gamma van varExp
resid_var <- (sigma_hat^2 * exp(2 * gamma_hat * Xvals_Xplus2)) # stap 4: model-implied residual variances 
...
resid_var <- sigma2 * exp(2 * (gamma[1] * Xvals_Xplus + gamma[2] * Xvals_Xplus2))
